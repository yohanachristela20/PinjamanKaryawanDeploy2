...
function Beranda() {
  ...
  const getPinjaman = async () =>{
    try {
      setLoading(true);
      const response = await axios.get("https://7a54-103-141-189-170.ngrok-free.app/pinjaman", {
        headers: {
  ...
  const getPinjamanData = async () =>{
    try {
      setLoading(true);
      const response = await axios.get("https://7a54-103-141-189-170.ngrok-free.app/pinjaman-data", {
        headers: {
          Authorization: `Bearer ${token}`,
      },
      // withCredentials: true,
