...
const Pelunasan = ({ showPelunasanModal, setShowPelunasanModal, angsuran, onSuccess }) => {
    ...
    const getPinjamanData = async (idPinjaman) => {
        try {
            const response = await axios.get(`http://10.70.10.144:5000/pinjaman/${idPinjaman}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
    ...
    const savePelunasan = async (e) => {
        ...
        try {
            ...
            const response = await axios.post('http://10.70.10.144:5000/angsuran', data, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
            ...
            if (statusBaru === 'Lunas') {
                const updatePinjamanResponse = await axios.put(
                    `http://10.70.10.144:5000/pinjaman/${id_pinjaman}/status`,
